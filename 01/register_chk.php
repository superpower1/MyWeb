...
//定义邮件内容
	$envelope="mrsoft@mrsoft.com";		//定义登录使用的邮箱
...
/*   网络版发送邮件方法  */
	...
	$config = array('auth' => 'login',
            ...
            'password' => 'mingri');				//定义SMTP的验证参数
	$transport = new Zend_Mail_Transport_Smtp('localhost', $config);		//实例化验证的对象
...
/*   网络版发送邮件方法  */	
	...
	$sql = "insert into reg_users(name,password,question,answer,email,realname,birthday,telephone,qq) values('".trim($_GET['name'])."','".md5(trim($_GET['pwd']))."','".$_GET['question']."','".$_GET['answer']."','".$_GET['email']."','".$_GET['realname']."','".$_GET['birthday']."','".$_GET['telephone']."','".$_GET['qq']."')";
